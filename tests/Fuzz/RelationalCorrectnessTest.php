...
final class RelationalCorrectnessTest extends DualDatabaseTestCase
{
    ...
    public function test_mutation_read_consistency_matches_oracle(): void
    {
        $this->eachSeed(function (int $seed, int $step): void {
            ...
            $mutations = $this->buildMutationPlan($userIds);
            ...
            // Compare: both sides should agree on which users exist and their attributes
            $actual = $this->snapshotUsers(User::whereKey($userIds)->get());
            $oracle = IdentityMap::disabled(fn (): array => ConnectionContext::using('test_b', fn (): array => $this->snapshotUsers(User::whereKey($userIds)->get())));
     ...
     *
     * @return array{
     *     users: list<array{id: int, name: string, email: string, active: bool}>,
     *     tags: list<array{id: int, name: string}>,
     *     posts: list<array{id: int, user_id: int, tag_id: int|null, title: string, published: bool}>,
    ...
    private function seedGraph(): array
    {
        ...
        foreach ($users as $u) {
            if (mt_rand(0, 1) === 1) {
                $c = Comment::create(['commentable_type' => User::class, 'commentable_id' => $u['id'], 'body' => "comment-u{$u['id']}"]);
                $comments[] = ['id' => $c->id, 'commentable_type' => User::class, 'commentable_id' => $u['id'], 'body' => $c->body];
            }
        }
        foreach ($posts as $p) {
            if (mt_rand(0, 1) === 1) {
                $c = Comment::create(['commentable_type' => Post::class, 'commentable_id' => $p['id'], 'body' => "comment-p{$p['id']}"]);
                $comments[] = ['id' => $c->id, 'commentable_type' => Post::class, 'commentable_id' => $p['id'], 'body' => $c->body];
            }
        }
    ...
    /**
     * @param  list<array{id: int, name: string, email: string, active: bool}>  $users
     * @param  list<array{id: int, name: string}>  $tags
     * @param  list<array{id: int, user_id: int, tag_id: int|null, title: string, published: bool}>  $posts
     * @param  list<array{id: int, commentable_type: string, commentable_id: int, body: string}>  $comments
     */
    private function mirrorToSecondary(array $users, array $tags, array $posts, array $comments): void
    {
        IdentityMap::disabled(function () use ($users, $tags, $posts, $comments): void {
            ConnectionContext::using('test_b', function () use ($users, $tags, $posts, $comments): void {
                foreach ($users as $u) {
                    User::forceCreate(['id' => $u['id'], 'name' => $u['name'], 'email' => $u['email'], 'active' => $u['active']]);
                }
                foreach ($tags as $t) {
                    Tag::forceCreate(['id' => $t['id'], 'name' => $t['name']]);
                }
                foreach ($posts as $p) {
                    Post::forceCreate(['id' => $p['id'], 'user_id' => $p['user_id'], 'tag_id' => $p['tag_id'], 'title' => $p['title'], 'published' => $p['published']]);
                }
                foreach ($comments as $c) {
                    Comment::forceCreate(['id' => $c['id'], 'commentable_type' => $c['commentable_type'], 'commentable_id' => $c['commentable_id'], 'body' => $c['body']]);
                }
            });
        });
    }
     ...
     * @return list<array{type: string, id: int, new_name: string|null}>
     */
    private function buildMutationPlan(array $userIds): array
    {
        $types = ['noop', 'rename', 'toggle-active', 'soft-delete', 'soft-delete-restore'];
        $plan = [];

        foreach ($userIds as $id) {
            $type = $types[mt_rand(0, 4)];
            $plan[] = ['type' => $type, 'id' => $id, 'new_name' => $type === 'rename' ? "Renamed-{$id}-".mt_rand(1, 99) : null];
        }
    ...
    /**
     * @param  array{type: string, id: int, new_name: string|null}  $mutation
     */
    private function applyMutation(array $mutation): void
    {
        ...
        match ($mutation['type']) {
            ...
            'rename' => (function () use ($user, $mutation): void {
                if (is_string($mutation['new_name'])) {
                    $user->name = $mutation['new_name'];
                    $user->save();
                }
            })(),
            ...
            default => throw new \InvalidArgumentException("Unknown mutation type: {$mutation['type']}"),
        };
    }
     ...
     * @return array<int, array{id: int, name: string, email: string, active: bool, deleted_at: string|null}>
     */
    ...
    /**
     * Snapshot user→posts→tag and user→comments for a set of user IDs.
     *
     * @param  list<int>  $userIds
